...
const getWinTemplate = (state, number) => {
...
const getLooseTemplate = (state, number) => {
  return `<tr>
    <td class="result__number">${number + 1}.</td>
      <td colspan="2">${getStatsTemplate(state.answersStats)}</td>
...
const getResultTemplate = (state, number) => {
  ...
  const resultTemplate = gameIsWon ? getWinTemplate(state, number) : getLooseTemplate(state, number);
